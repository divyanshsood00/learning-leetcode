...
async function loadActiveTabData() {
    ...
    try {
        ...
        if (chrome.runtime.lastError) {
            setError('Cannot reach page script. Make sure you are on a LeetCode problem page (https://leetcode.com/problems/...).\n\n' +
 ...
 */
function startTimerLoop(activeTabId) {
...
function updateTimeForTab(tabId) {
    ...
    chrome.storage.local.get(['titleTimers', 'tabToTitle'], (res) => {
        ...
        let elapsedMs = 0;
        if (typeof timer.accumulated === 'number')
            elapsedMs += timer.accumulated;
        if (typeof timer.runningSince === 'number')
            elapsedMs += (Date.now() - timer.runningSince);
...
function initializeLastQuestionButton() {
...
async function getLastUsedProblem() {
    return new Promise((resolve) => {
...
async function updateLastQuestionButton() {
    const lastQuestionBtn = document.getElementById('lastQuestionBtn');
    if (!lastQuestionBtn)
        return;
    try {
...
async function loadAndDisplayStats() {
    try {
        ...
        if (totalProblemsEl)
            totalProblemsEl.textContent = stats.totalProblems.toString();
        if (totalTimeEl)
            totalTimeEl.textContent = formatSeconds(Math.floor(stats.totalTimeSpent / 1000));
        if (completedProblemsEl)
            completedProblemsEl.textContent = stats.completedProblems.toString();
        if (recommendationsCountEl)
            recommendationsCountEl.textContent = recommendations.length.toString();
...
async function getProblemStats() {
    return new Promise((resolve) => {
        chrome.storage.local.get(['leetcodeProblems'], (result) => {
            ...
            const totalTimeSpent = problems.reduce((sum, problem) => sum + (problem.timeSpent || 0), 0);
            const averageTimePerProblem = totalProblems > 0 ? totalTimeSpent / totalProblems : 0;
            const mostUsedProblem = problems.reduce((most, current) => (current.timeSpent || 0) > (most.timeSpent || 0) ? current : most, problems[0] || null);
            resolve({
                totalProblems,
                totalTimeSpent,
...
async function getRandomRecommendationByTag(tag) {
    try {
...
function initializeRandomRecommendationButton() {
...
function initializeRandomRevisionButton() {
...
async function getRandomRevisionProblem() {
    return new Promise((resolve) => {
        chrome.storage.local.get(['leetcodeProblems'], (result) => {
            const storage = result.leetcodeProblems;
