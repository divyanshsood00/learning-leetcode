...
function extractTags() {
    ...
    const headerCandidates = Array.from(document.querySelectorAll('h2,h3,h4,div')).filter(el => /topics|tags|related topics|related/i.test((el.textContent || '')));
...
function reportTitleToBackground() {
    const title = document.title || null;
    chrome.runtime.sendMessage({ action: 'reportTitle', title }, (response) => {
...
// Save problem data to storage
async function saveProblemData(problemData) {
    try {
        // Check if ProblemStorageManager is available
        if (typeof window.ProblemStorageManager !== 'undefined') {
            await window.ProblemStorageManager.saveProblem(problemData);
...
// Generate problem ID
function generateProblemId(problemData) {
...
async function handleSuccessfulSubmission() {
    ...
    try {
        ...
        if (typeof window.ProblemStorageManager !== 'undefined') {
            await window.ProblemStorageManager.markProblemCompleted(currentProblemId);
...
async function fetchAndStoreSimilarQuestions() {
    if (!currentProblemId)
        return;
    try {
        ...
        if (typeof window.ProblemStorageManager !== 'undefined') {
            await window.ProblemStorageManager.addRecommendations(currentProblemId, mockSimilarQuestions);
        }
    }
    catch (error) {
